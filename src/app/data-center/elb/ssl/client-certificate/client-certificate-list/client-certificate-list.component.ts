...
export class ClientCertificateListComponent implements OnInit {
  ...
  getsslcert(){
    this.elbService.getSSLCertificate(this.params).subscribe(data => {
      this.clientcertlist = data.sslcerts.filter(cert => {
        return cert.certificatetype.indexOf('CLNT_CERT') !== -1
      });
      this.clientcertlist.filter(function(obj){
      ...
      this.totalResult = this.clientcertlist.length;
      this.clientcertlist = this.clientcertlist.slice(0, 16);
      this.loading = false;
      this.certCallError = false;
      if(this.clientcertlist.length === 0){
